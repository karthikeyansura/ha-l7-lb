...
| Task | Description |
|------|-------------|
| 2.1 Run round-robin baseline | Deploy with `policy: round-robin`, run `AlgorithmCompareUser` at 100, 500, 1000, 2000 users for 5 min each |
| 2.2 Run least-connections comparison | Redeploy with `policy: least-connections`, repeat identical user counts and durations |
| 2.3 Run weighted comparison | Redeploy with `policy: weighted`, repeat identical user counts and durations |
| 2.4 Collect metrics snapshots | Capture LB metrics endpoint (port+1000) at each run for per-backend latency percentiles |
...
Assumes approximately 4-5 weeks remaining (March 28 -- April 30, 2026).
...
| Apr 6 -- Apr 12 | Experiment 3 Execution and Data Analysis Begins | All horizontal scaling runs complete (1/2/4/8 LB instances, baseline + spike). CSV processing scripts written. Experiment 1 and 2 charts drafted. |
| Apr 13 -- Apr 19 | Data Analysis Complete, Report Draft | All charts finalized. Experiment 3 scaling efficiency analysis done. Full report draft (introduction through conclusions). |
| Apr 20 -- Apr 26 | Final Report, Presentation, Submission | Report polished and submitted. Presentation slides complete. Demo rehearsed. AWS resources destroyed. |
...
- **Apr 19:** Report draft complete for peer review between team members.
- **Apr 26:** Final submission deadline.
