<!-- Generated: 2026-03-29 | Files scanned: 22 | Token estimate: ~650 | Last commit: 64e995f -->
...
```
cmd/
  lb/main.go         (281 lines) — entry point, wiring, metrics HTTP server, graceful shutdown
  backend/main.go    (109 lines) — test backend with chaos injection headers

internal/
  config/config.go   (100 lines) — YAML + env override (REDIS_ADDR, REDIS_PASSWORD)
  proxy/proxy.go     (349 lines) — L7 reverse proxy, body buffering, retry budget, idempotent retry
...
Total: 3,210 lines Go (1,144 test lines).
