...
**Sai Karthikeyan Sura** -- Primary author. Built the entire LB (PRs #1-61), all Terraform modules, and led the AWS deployment.
**Zhaoshan "Joshua" Duan** -- Contributor. Early bug fixes, documentation, heterogeneous backend experiments, and ECS API discovery investigation.
...
**Done**: Core implementation (Mar 16), hardening PRs #25-61 (Mar 16-28), AWS deployment (Mar 28-29), Exp 1 runs in progress (Mar 29). **Planned**: Exp 2 (Mar 31-Apr 5), Exp 3 (Apr 6-12), final report (Apr 13-19).

**Sai**: All LB code and PRs, Terraform, AWS deployment, Exp 1 on homogeneous backends, Exp 3. **Joshua**: Early fixes, PR review, ECS API discovery investigation (abandoned -- Learner Lab blocks Cloud Map, pivoted to personal AWS), heterogeneous backend deployment, Exp 1 RR+LC on strong/weak backends, Exp 2, documentation and report.
...
1. **Raft KV Store with Chaos Engineering** (Qian Li, Zhengyi Xu, Wenyu Yang, Siwen Wu) -- *Similarity*: Both are distributed Go systems with fault injection testing and recovery time measurement under load. *Difference*: Their system requires consensus (CP -- reject writes during partition); ours is AP -- the LB keeps serving with stale health state, preferring availability over coordination.
2. **Onion Routing Network on ECS Fargate** (Arjun Avadhani, Rahul Suresh) -- *Similarity*: Both build custom Go network infrastructure on ECS Fargate with task failure testing, horizontal scaling experiments, and Locust + CloudWatch instrumentation. *Difference*: Their relays operate independently once a circuit is built (no shared state); ours coordinate via centralized Redis Pub/Sub.
3. **[Project C]** -- *Similarity*: [How it relates]. *Difference*: [How ours differs].

---

## 5. Methodology

`Client → NLB (L4) → LB ECS Tasks (L7) → Backend ECS Tasks`, with ElastiCache Redis for Pub/Sub state sync and Cloud Map DNS for discovery (5s poll). The proxy retries idempotent methods (GET/PUT/DELETE) with a 20% retry budget cap. Redis is optional -- the LB degrades to local-only health state if unavailable. Experiments run on both homogeneous (Sai) and heterogeneous backends (Joshua: 1 strong 512 CPU + 1 weak 256 CPU).
...
### Exp 1 — Homogeneous Backends (Sai)

<!-- TODO: Sai to add RR vs LC results on identical backends -->
...
LC provides modest tail latency improvement but the LB was the bottleneck at ~99% CPU, limiting observable differences. Weak backend ran ~71% CPU vs. strong at ~37% under both algorithms. Weighted is deferred -- single DNS query assigns equal weights to all backends.
...
**Worst case**: All backends fail simultaneously -- every request attempts, fails, retries, fails again (504). The 20% retry budget prevents cascading amplification. **Base case**: All healthy, LB adds <1ms overhead.
